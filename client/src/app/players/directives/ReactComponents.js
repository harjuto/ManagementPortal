...
.factory("PlayerComponents", function(){
              ...
              <div className="row">
                <div className="col-md-12">
                    <self.FilterBar
                        ...
                        onFilterUpdate={this.handleFiltering}
                    />
                  </div>
                  <div className="col-md-12">
                    <self.List
  ...
  PlayerComponents.List = React.createClass({
      render: function() {
          ...
          for(var i = 0 ; i < Math.min(this.props.tableState.showCount, this.props.players.length) ; i++){
            ...
            rows.push(<self.Row player={player} key={player.id} />);
          }
          return (
      ...
      render: function(){
        return(
          <form className="form-inline">
            <self.FilterTextInput text={this.props.tableState.filterText} handleChange={this.changeFilterText}/>
            <self.FilterShowCount count={this.props.tableState.showCount} handleChange={this.changeShowCount}/>
          </form>
        )
      }
  });
  ...
  PlayerComponents.FilterShowCount = React.createClass({
    ...
    render: function(){
      return(
        <div className="form-group">
          <label for="sel1">Show count:</label>
          <select className="form-control" id="sel1" onChange={this.handleChange} ref="showCountInput">
            <option>10</option>
            <option>50</option>
            <option>250</option>
            <option>500</option>
      ...
      render: function() {
          return (
            <tr>
              ...
              <td>{this.props.player.dateCreated}</td>
              <td>{this.props.player.lastLogin}</td>
            </tr>
          );
      }
  });
